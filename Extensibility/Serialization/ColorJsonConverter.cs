...
namespace BazthalLib.Extensibility.Serialization
{
    public class ColorJsonConverter : JsonConverter<Color>
    {
        ...
        public override Color Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            DebugUtils.Log("ColorJsonConverter", "Read", $"Input: {str}", logLevel: LogLevel.Info);

            if (string.IsNullOrWhiteSpace(str))
            {
                DebugUtils.Log("ColorJsonConverter", "Warning", "Color string was empty or null.", logLevel: LogLevel.Info);
            ...
            {
                DebugUtils.Log("ColorJsonConverter", "Error", $"Failed to parse color \"{str}\" — {ex.Message}. Using fallback.", logLevel: LogLevel.Error);
